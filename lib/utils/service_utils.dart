...
import 'package:tonic/modules/root/update_dialog.dart';

import 'package:tonic/provider/database/database.dart';
import 'package:tonic/services/dio/dio.dart';
import 'package:tonic/services/logger/logger.dart';

import 'package:tonic/utils/primitive_utils.dart';
  ...
  static Future<void> checkForUpdates(
    ...
    if (Env.releaseChannel == ReleaseChannel.nightly) {
      final value = await globalDio.getUri(
        Uri.parse(
          "https://api.github.com/repos/tonic-player/tonic/actions/workflows/tonic-release-binary.yml/runs?status=success&per_page=1",
        ),
        options: Options(
          responseType: ResponseType.json,
        ),
      ...
      final value = await globalDio.getUri(
        Uri.parse(
          "https://api.github.com/repos/tonic-player/tonic/releases/latest",
        ),
      );
